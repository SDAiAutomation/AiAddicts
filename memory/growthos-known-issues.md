...
- Monorepo: growthos shares git history with the FI Validator RAG (repo root) and faceless-kids-stories; consider splitting it out once la loop est validée.

**Passage à plusieurs users — pas prêt (2026-09-05).** Vérifié dans le code (`grep` sur `engine/*.py` + `worker.py`) : aucune lecture/écriture de `credits_balance`/`credits_ledger` nulle part côté pipeline Python — confirme et précise le point ligne 40 (table non branchée, pas juste "inutilisée"). Deux autres trous liés au multi-tenant, pas encore corrigés :
- `worker.py` traite la file `content_items` de **toutes** les organisations en FIFO global, un item à la fois (~75s/vidéo mesuré sur un run réel à 5 blocs) — pas de priorité/équité par org. `repo.claim_queued_item()` est déjà race-safe (update conditionné au statut), donc lancer plusieurs process `worker.py` en parallèle marcherait sans doublon, juste jamais fait.
- `ELEVENLABS_API_KEY`/`PEXELS_API_KEY` sont des clés globales dans `.env`, partagées par toutes les orgs — pas de quota par tenant, un org qui génère beaucoup peut faire échouer (429) les générations des autres.
À faire avant d'ouvrir à plusieurs clients payants : brancher `credits_ledger` dans `assembler.run_for_content_item()` (décrémenter + refuser si solde ≤ 0), puis need décider si on scale le worker horizontalement (plusieurs process) ou si un throughput séquentiel suffit pour le volume visé.
