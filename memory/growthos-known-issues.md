...
- **Stockage vidéo** : `content_items.video_url` est un chemin de fichier local à la machine qui a fait tourner `worker.py`, jamais uploadé nulle part (choix de portée MVP délibéré — voir README §1, publication manuelle par l'opérateur). Pas une URL partageable : seul l'opérateur du worker peut voir la vidéo produite. Ajouter un upload vers Supabase Storage est possible mais pas fait (bucket, policies, coût de stockage à décider avec l'utilisateur avant de coder) — voir [[growthos-web-frontend]].
